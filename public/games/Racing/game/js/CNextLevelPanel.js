function CNextLevelPanel(iTimeElaps, iScore, iLevel){
    ...
    this._init = function(iTimeElaps, iScore, iLevel){
        ...
        // var apiUrl = "https://4bdeotpg41.execute-api.ap-south-1.amazonaws.com/prod";
        // try { session_id = localStorage.getItem('userid'); } catch(e){}
        // try { email_id   = localStorage.getItem('email');  } catch(e){}
        // try { username   = localStorage.getItem('username'); } catch(e){}
        // var body = {'sessionId': session_id, 'username': username, 'email': email_id, 'gameId': 23, 'score': iScore, 'gameName': 'Car Race'};
        // console.log('Saving score to db...');
        // fetch(apiUrl + "/save-score", {
        //     method: 'POST',
        //     headers: { 'Content-Type': 'application/json' },
        //     body: JSON.stringify(body)
        // }).then(function(res){ return res.json(); })
        //   .then(function(data){ console.log(data); })
        //   .catch(function(error){ console.warn('Score save unavailable:', error.message); });
        this._sendScore(); 
    ...
    this._onRestart = function(){
        ...
        new createjs.Tween.get(_oPanelContainer).to({y:_pStartPanelPos.y},400, createjs.Ease.backIn).call(function(){
            _oParent.unload();
            s_oGame.nextLevel();  // go to next level instead of crashing
        }); 
    };
