function CMenu(){
    ...
    this._init = function(){
        ...
        if(s_oLocalStorage.isDirty()){
            ...
            _pStartPosDelete = {x: (oSprite.width/2) + 12, y: (oSprite.height/2) + 16};            
    ...
    this.refreshButtonPos = function(iNewX,iNewY){
        if(DISABLE_SOUND_MOBILE === false || s_bMobile === false){
            if(_oAudioToggle){ _oAudioToggle.setPosition(_pStartPosAudio.x - iNewX,iNewY + _pStartPosAudio.y); }
        }
        if(_oCreditsBut){ _oCreditsBut.setPosition(_pStartPosCredits.x + iNewX,iNewY + _pStartPosCredits.y); }
        if (_fRequestFullScreen && screenfull.isEnabled){
            if(_oButFullscreen){ _oButFullscreen.setPosition(_pStartPosFullscreen.x + iNewX, _pStartPosFullscreen.y + iNewY); }
        }
        if(_oButDelete && s_oLocalStorage.isDirty()){
